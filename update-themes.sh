...
WORKING_DIR=/home/theringmaster/Projects/Tesla/Build/Official/
...
function copy_all () {
  ...
  do
    cd $WORKING_DIR/packages/apps
    cp -r ${array[i]} $WORKING_DIR/TeslaScripts/themes-resources/packages/apps/
  done
}

declare -a root=('TeslaScripts' 'abi' 'bionic' 'art' 'bootable' 'build' 'dalvik' 'development' 'device' 'external' 'frameworks' 'hardware' 'kernel' 'libcore' 'libnativehelper' 'manifest' 'ndk' 'out' 'packages' 'pdk' 'prebuilts'
...
declare -a values=('all_search_engines.xml' 'appmsg_colors.xml' 'arrays.xml' 'attrs_manifest.xml' 'bookmarks_icons.xml' 'bools.xml' 'config.xml' 'defaults.xml' 'donottranslate.xml' 'donottranslate_config.xml'
                   ...
                   'donottranslate-search_engines.xml' 'tesla_arrays.xml' 'tesla_strings.xml' 'tesla_symbols.xml' 'fractions.xml' 'ids.xml' 'integers.xml' 'internal.xml' 'keys.xml' 'lland_config.xml' 'lland_strings.xml' 'plurals.xml'
...
declare -a theme_packages=('Calculator' 'Calendar' 'CalendarWidget' 'Camera2' 'Contacts' 'ContactsCommon' 'DUI' 'TeslaCoil' 'SlimOTA' 'DeskClock' 'Dialer' 'Email' 'Gallery2' 'InCallUI'
                           'Launcher3' 'LockClock' 'Messaging' 'OmniSwitch' 'PhoneCommon' 'Settings' 'Stk' 'KernelAdiutor')
...
git clone https://github.com/TeslaRom-N/TeslaScripts.git
...
cd values
delete_useless ${values[@]}
rm -rf  $WORKING_DIR/TeslaScripts/themes-resources/frameworks
cd $WORKING_DIR
process_all ${theme_packages[@]}
echo "Cleaning target folders"
rm -rf TeslaScripts/themes-resources/packages
mkdir TeslaScripts/themes-resources/packages
mkdir TeslaScripts/themes-resources/packages/apps
mkdir TeslaScripts/themes-resources/frameworks
echo "Copying all files to $WORKING_DIR/TeslaScripts/themes-resources"
copy_all ${theme_packages[@]}
cp -r $WORKING_DIR/frameworks/base/core $WORKING_DIR/frameworks/base/packages $WORKING_DIR/TeslaScripts/themes-resources/frameworks
cd /home/theringmaster/Downloads/TeslaScripts/themes-resources